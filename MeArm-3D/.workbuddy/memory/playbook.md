...
CDP 脚本 / `tmp_probe` 包）已全部清理。**（下一轮已把三条路里的第 1 + 3 条落地，见 §14.9。）**

### 14.9 已落地实现（2026-09-16）—— 两条上报路径 + `origin` 标注

**协议层分流（这是核心，别把它当细节）**

| 来源 | 设备回吐 | 语义 | 后端标注 |
|---|---|---|---|
| `JR` 受理 | `STATE …`（关节角，位置推进逐帧） | **命令引起** | `origin=command` |
| `SET` / `JOY` / `S<n>=` | `# SERVO S6=.. S7=.. S8=.. S9=..`（舵机角，异步事件） | **设备侧外部变化** | `origin=device` |

- `OK JR`（目标角）/ `OK SET`（钳位后目标角）/ `STATUS`·`# SERVO`（**实际角**）三者形状都含
  `S<n>=` ⇒ 必须靠**前缀白名单**分流（`#` / `STATUS` → `ReplyServo`），
  否则"状态永远等于命令、误差恒为 0"。
- **`# ` 前缀 = 异步事件**，天然不参与"每条指令恰好回一行"的应答契约
  ⇒ 拿它上报状态**不会**放行某条在途命令的 ACK 门控。这是它能安全插进链路的根本原因。
- **跟随必须由 `origin` 决定，前端禁止猜**。猜"Actual ≠ Command 就跟随"会在拖动
  （设备仍在斜坡）时把命令侧拉回半路；猜"一律跟随"会吃掉命令语义。
  未知 `origin` 一律按 `command` 降级（宁可少跟随）。
- **跟随必须抑制下发**（`suppressCommandSend` 只覆盖那一次 `set`）：否则每次设备上报都会
  反弹一条命令下来，形成 `设备 → 上位机 → 设备` 的对抗。

**低优先级的两端落点**（用户铁律：自主上报让位于交互指令，且**让位 ≠ 丢弃**）

| 端 | 闸门 | 被挡下时 |
|---|---|---|
| 固件 | `arm_report_tick()` 排在 `cmd_poll()` **之后**，且 `uart_tx_used()==0`（TX 环全空）才发 | 保留脏标志，**发送那一刻才取值** ⇒ latest-wins |
| sim | `jrBusy` 在途标志 | 只置 `reportDirty`；命令了结后 `flushReport()` 补报**当时**的实际值 |

**仿真侧对等实现**：sim 新增 `SET` / `S<n>=` / `JOY` / `STATUS`（与固件**同一公式**：
`beyond = raw>800 ? raw-800 : 200-raw`；`step = 2 + beyond/30` clamp ≤10；`id==8` 方向取反）。
⇒ 无硬件即可端到端验证「摇杆 → 界面跟随」。
另加 WS `{"type":"device_command","line":"JOY 7 0"}` 直通（`controller.RawLine`：
**不做限位校验、不参与 ACK 门控**，语义 = 有人把线直接接在设备上敲命令）。

**验收**：`core/tools/verify_device_follow.mjs`（**仿真与真机同一份脚本**，只差 `--config`）
`--config sim` ⇒ PASS 23 / FAIL 0，连跑 3 次稳定。固件 FLASH 12848 → **13104 B**（+256 B），
RAM 844 → **849 B**，零警告。

### 14.10 ★★ 可复用的教训（换个项目仍成立）

1. **e2e 脚本证明"可观测后果"，单测证明"闸门本身"，别混。**
   竞态/优先级类机制的窗口常常窄到脚本摸不着（这里 sim 的在途窗口只有 `LatencyMs`=15ms，
   且 `deliver()` 到期会用**绝对目标**覆盖舵机目标 ⇒ 窗口内的拨动整个被丢掉，
   那是"延迟期内不生效"的**建模选择**，不是上报缺陷）。
   ⇒ 闸门用单测**直接构造在途态**来证；脚本只证"命令优先 + 被让位的拨动没丢"。

2. **验收脚本必须自己造出"可观测的位移"，否则会把"本来到位"读成"失败"。**
   命令是**有限角速度逼近**，目标 == 当前位置 ⇒ **零位移 ⇒ 零状态帧** ⇒ "等收敛"拿到
   `NaN`，看起来像"命令没被受理"。同类坑还有"贴着限位下发"（固件钳位后同样零位移）。
   ⇒ 下发前必须（a）选一个离当前位置 ≥ 阈值的**且夹进限位内**的目标
   （候选取值不夹限位会落到限位外 ⇒ 被 `ERR JOINT` 同步拒掉 ⇒ 同样零状态帧）；
   （b）已经在目标位就**跳过**下发而不是判 FAIL。

3. **造前提的参数要从真值里挑，不能写死常量。**
   实例：摇杆方向如果写死"向下拨"，而肩关节 HOME `0.85°`、限位 `[-6.09, 49.45]`，
   向下只有 **6.9°** 行程 ⇒ 第一次之后就被限位钳住，脚本报"没收到多帧上报"，
   把**限位**造成的现象误报成**上报机制**的缺陷。
   ⇒ 方向从 `hello.model.limits` 里挑**行程更大的一侧**；次数按"剩余行程 × 0.4"算。

4. **一条断言只证一件事，否则失败无法归因。**
   本轮把并发拆成三段：**干净基线收敛** / **正向并发**（命令在途插入设备拨动）/ **反向抢占**
   （设备在坡上时发命令）。第一版把三者混成一段，于是
   "命令的斜坡被设备输入接手后改走 `# SERVO` ⇒ 看不到收敛帧"被读成"命令没收敛"（假 FAIL）。

5. **别把"正确的语义"写成断言。**
   命令之后设备又拨了 ⇒ 位置本来就该是"命令目标 + 拨动"，设备对物理执行器有最后发言权。
   在这条链上断言"收敛到命令目标"会制造假 FAIL。先写清"这一段证明什么"，再写断言。
...
- **正文 < 10k 字符**（2026-09-16 起由 11k 收紧）；超了就把细节按节拆进 `references/*.md`，
  正文只留核心 + 指针表。
- **内容判据（2026-09-16 确立，比"体积"更重要）**：只保留**经验型**内容 ——
  即"换个项目仍然成立"的踩坑规律、判据、配方。必须移除两类：
  ① **记录型**：一次性的版本快照（FLASH/RAM 具体字节）、一次性的测试计数（`196 passed`）、
     「本次实测于某日」的标注。**承载结论的部分要保留**（如"双构零 warning"、"只看与上一版趋势"），
     把绝对值改判据、把数字改占位符。
  ② **项目绑定型**：具体项目/包名（`mearm-v1` → `<pkg-id>`）、本仓绝对路径、
     本仓专属回归测试名。**举例可以留、专名要泛化**。
  ⚠️ 例外：**「用户铁律于某日确立」这类日期是规则出处，要留**（它解释"为何不可违反"）。
- **拆分纪律**：reference 只把标题降一级（`##`→`#`），**正文逐字节不变**；
  拆完必须跑两项校验 —— ① 代码块围栏数原文 == 新正文 + refs 之和；
  ② 原文每个**非标题行**都能在「新正文 ∪ refs」里找到；再查**引用双向**（无死链、无孤儿文件）。
  ⚠️ 若拆分时**故意泛化了项目专名**，②的"缺失行"就是那些被改写的行 —— 需逐条确认
  "缺失的都是我主动改的"，而不是真有内容丢失。两种原因混在同一次比对里，
  是某会话差点误判为"内容丢失"的地方。
...
- ★ **计量 description 必须用真 YAML 解析器**（`yaml.safe_load`），**不能用正则取首行** ——
  正则会把折叠式的后续行漏掉，实测因此把总量 **10145 少报成 8752（漏 1377）**，
  还误判 4 个 skill 为"description 仅 1 字符"。
- ★ **拆分后必须补一段 refs 索引**（正文末尾「本 skill 的参考文件」指针表）。
  实测：拆完 22 个 refs 文件，其中 **14 个曾是"孤儿"**（孤立文件存在但正文零引用），
  纯靠"围栏守恒 + 零行丢失"两项校验**查不出来** —— 必须显式跑引用双向。
- ★ 本项目 5 个重 skill 曾按此拆过（2026-09-15）；**全库 31 个 skill 的归一化在 2026-09-16 完成**：
  正文全部 < 10k（最大 9811）、description 全部单行 ≤500（最大 468）、零项目绑定、零记录型残留、
  24 个 refs 全部被引用。注意：`~/.workbuddy/skills/` **不在 git 仓库内**，
