...
**最近一次全绿概要**（2026-09-17 全量重编后实跑 `tsc` / `vitest` / `go vet` / `go test` / `pytest` / Sim2Sim / 三条链路 e2e）：
`tsc` 0 error · `vitest` **442 passed**（`Test Files 33 passed`）·
`go vet` 干净 · `go test` **114 个顶层用例**全绿（6 包 · 0 FAIL，另有 7 个子测试）·
`arm-web` `go test` **39 passed** · `pytest` **214 passed**
...
`run_sim2sim.py --all` 覆盖选择器声明的**全部**机器人（现为 mearm-v1 —— `so-arm101` 已于 `178d1f0` 移除）·
固件（AVR）FLASH **13116 B**（text 12978 + data 138）/ RAM **849 B**（.data 138 + .bss 711），零警告 ·
sim2sim 三侧面 **≤7.65e-14 mm**（容差 1e-6 ⇒ 余量 **1.3e7**，本轮 40 组用例）。
（跨批 7 seed / 272 组的 **≤8.04e-14 mm**、`pytest` 分项计数、`verify_device_follow.mjs` 各项为 2026-09-16 实测，本轮未重跑；
本轮重编后复跑新增的读数：`run_sim2sim.py --all` FK **40/40** · IK 25/28（3 项按设计拒绝）·
`verify_external_origin.mjs` **3/3** · `sim2sim-check.mjs` **30/30** · `external-origin-follow.mjs` **9/9**。
固件 FLASH 由 README 原记的 13104 → **13116**：13104 实测为 `ce44827` **之前**那版源码的产物，README 落后一个提交。）
